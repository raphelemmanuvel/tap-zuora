...
def poll_job_until_done(
    job_id: str, client: Client, api: Union[Type[apis.Rest], Type[apis.Aqua]]
) -> List:
...
def handle_aqua_timeout(ex: apis.ExportTimedOut, stream: Dict, state: Dict):
    ...
    state["bookmarks"][stream["tap_stream_id"]]["current_window_end"] = (
...
def handle_rest_timeout(
    ex, stream: Dict, state: Dict, current_window: int, start_pen: str
) -> int:
    if stream.get("replication_key"):
        LOGGER.info("Export timed out, reducing query window and writing state.")
